...
const MisPedidos = () => {
  ...
  return (
    <div className="max-w-md mx-auto px-4 py-8 bg-white shadow-lg rounded-lg">
      <h1 className="text-4xl font-bold text-green-700 text-center mb-6">Mis Pedidos</h1>
      {orders.length === 0 ? (
        <p className="text-center text-gray-700">No tienes pedidos realizados.</p>
      ) : (
        <ul className="space-y-4">
          {orders.map((order) => (
            <li key={order.id} className="border rounded-lg p-4">
              <h2 className="text-lg font-medium">Pedido ID: {order.id}</h2>
              <p className="text-gray-700"><strong>Total:</strong> ${order.totalAmount}</p>
              <h3 className="text-md font-medium">Platos:</h3>
              <ul className="list-disc pl-5">
                {order.dishes.map((dish, index) => (
                  <li key={index} className="text-gray-700">{dish.name} - ${dish.price}</li>
                ))}
              </ul>
            </li>
          ))}
